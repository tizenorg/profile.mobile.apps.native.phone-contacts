	...
	: m_Group(group)
{
	...
	m_GroupChangeCbHandle = m_Group->setChangeCallback(std::bind(&BasicInfoItem::onGroupChanged, this, _1));
}
...
void BasicInfoItem::onGroupChanged(int type)
{
	...
	} else if (type & LogGroup::ChangeImage) {
		elm_genlist_item_fields_update(getObjectItem(), PART_THUMBNAIL, ELM_GENLIST_ITEM_FIELD_CONTENT);
	} else if (type & LogGroup::ChangeCount) {
		m_Log = m_Group->getLogList().back();
	} else if (type & LogGroup::ChangeRemoved) {
		m_Group = nullptr;
	}
}
