	...
	: m_Group(group), m_Genlist(nullptr), m_BasicInfoItem(nullptr), m_GroupItem(nullptr)
{
	...
	m_Group->setLogRemoveCallback(std::bind(&DetailsView::onLogRemoved, this, _1));
	m_GroupChangeCbHandle = m_Group->setChangeCallback(std::bind(&DetailsView::onGroupChanged, this, _1));
}

DetailsView::~DetailsView()
{
	if (m_Group) {
		m_Group->unsetChangeCallback(m_GroupChangeCbHandle);
	}
}
...
void DetailsView::onLogRemoved(Log *log)
{
	int logId = log->getId();
	auto it = find_if(m_LogItems.begin(), m_LogItems.end(), [logId](LogDetailItem *logItem) {
		return logItem->getLogId() == logId;
	});
	if (it != m_LogItems.end()) {
		onItemRemove(*it);
		delete *it;
		m_LogItems.erase(it);
	}
