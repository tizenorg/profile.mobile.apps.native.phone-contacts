...
LogGroupItem::LogGroupItem(const tm &date)
	: m_Date(date)
{ }
...
char *LogGroupItem::convertDate(const tm &date)
{
	...
	if (date.tm_year == nowDate.tm_year && date.tm_mon == nowDate.tm_mon) {
		if (date.tm_mday == nowDate.tm_mday) {
			return strdup(_("IDS_LOGS_HEADER_TODAY"));
		} else if ((date.tm_mday + 1) == nowDate.tm_mday) {
