...
LogGroup *LogProvider::addLog(LogGroupList &logGroupList, Log &log)
{
	...
	if (shouldGroupLogs(log, lastLogGroup->getFirstLog())) {
...
bool LogProvider::onGroupChanged(GroupIterator &groupIt)
{
	...
	if (group->getLogList().empty()) {
		group->setChangedType(LogGroup::ChangeRemoved);
		group->onChange();
	...
	CONTACTS_LIST_FOREACH(list, record) {
		...
		if (changeType == CONTACTS_CHANGE_UPDATED) {
			...
			for (auto &&group : m_Groups) {
				const Log &log = group.get()->getFirstLog();
				...
				if (log.getContactId() == id) {
					...
					if (strcmp(name, logName) != 0) {
						group.get()->setChangedType(LogGroup::ChangeName);
						group->onChange();
					}

					if (isImageChanged) {
						group.get()->setChangedType(LogGroup::ChangeImage);
						group->onChange();
					}
				}
			}
