...
void ContactRecordProvider::insertContactData(contacts_record_h record)
{
	auto contact = static_cast<ContactRecordData *>(createContactData(record));
...
void ContactRecordProvider::onChanged(ContactDataList::iterator contactIt, int id, contacts_changed_e changeType)
{
	...
	if (changeType == CONTACTS_CHANGE_UPDATED) {
		...
		if (record) {
			...
			if (shouldUpdateChangedCallback()) {
...
void ContactRecordProvider::updateChangedCallback(ContactDataList::iterator it)
{
	...
	contactData->unsetChangedCallback();
	contactData->setChangedCallback(
			std::bind(&ContactRecordProvider::onChanged, this, it, _1, _2));
}
