...
bool ContactIntField::isChanged() const
{
	return m_InitialValue != getValue();
}

int ContactIntField::getValue() const
{
	return getRecordInt(getRecord(), getPropertyId());
}

void ContactIntField::setValue(int value)
{
	contacts_record_set_int(getRecord(), getPropertyId(), value);
}

void ContactIntField::onInitialize(contacts_record_h record)
{
