...
{
	size_t groupRelCount = m_GroupRels.getFieldCount();
	std::vector<bool> isSelected;
	isSelected.reserve(groupRelCount);

	for (auto &&result : results) {
		Group *group = (Group *) result.value.data;

		bool isFound = false;
		for (size_t i = 0; i < groupRelCount; ++i) {
			auto &field = *m_GroupRels.getField(i);
			if (getRecordInt(field.getRecord(), _contacts_group_relation.group_id) == group->getId()) {
				isSelected[i] = true;
				isFound = true;
				break;
			}
	...
	for (size_t i = 0; i < groupRelCount; ++i) {
		if (!isSelected[i]) {
			m_GroupRels.removeField(*m_GroupRels.getField(i));
		}
	}
