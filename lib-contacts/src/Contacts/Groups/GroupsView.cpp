...
void GroupsView::onMenuPressed()
{
	...
	menu->addItem("IDS_LOGS_OPT_DELETE", [this] {
		...
		setSelectCallback([this](SelectResults results) {
			...
			popup->addItem("IDS_PB_OPT_GROUPS_ONLY_ABB",
					std::bind(&GroupsView::onDeleteSelected, this, results, false));
			popup->addItem("IDS_PB_OPT_GROUPS_AND_MEMBERS_ABB",
					std::bind(&GroupsView::onDeleteSelected, this, results, true));
...
void GroupsView::onDeleteSelected(SelectResults results, bool shouldDeleteMembers)
{
...
void GroupsView::deleteGroups(SelectResults results, bool shouldDeleteMembers)
{
	...
	for (auto &&result : results) {
		groupIds.push_back(result.value.id);
	}

	if (shouldDeleteMembers) {
		deleteMembers(groupIds);
	}
	contacts_db_delete_records(_contacts_group._uri, groupIds.data(), groupIds.size());

	contacts_disconnect_on_thread();
}

void GroupsView::deleteMembers(std::vector<int> groupIds)
{
	...
	contacts_db_delete_records(_contacts_contact._uri, contactIds.data(), contactIds.size());
