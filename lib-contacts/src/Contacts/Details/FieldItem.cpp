...
bool FieldItem::onLongpressed()
{
	...
	if (id != FieldNumber && id != FieldEmail && id != FieldAddress &&
			id != FieldMessenger && id != FieldUrl && id != FieldNote) {
		return false;
	}
	...
	menu->addItem("IDS_TPLATFORM_OPT_COPY", [this] {
		...
		auto window = getParent()->findParent<Ui::Window>();
		elm_cnp_selection_set(window->getEvasObject(), ELM_SEL_TYPE_CLIPBOARD, ELM_SEL_FORMAT_TEXT, text, strlen(text));
	});
