...
PersonProvider::PersonProvider(int filterType)
	: m_FilterType(filterType), m_IsFilled(false), m_IsUpdateEnabled(false), m_IsUpdatePending(false)
{
...
contacts_record_h PersonProvider::getPersonRecord(int property, int value) const
{
	contacts_query_h query = getQuery();
	contacts_filter_h filter = getFilter();
	if (!filter) {
	...
	contacts_filter_add_int(filter, property, CONTACTS_MATCH_EQUAL, value);
	contacts_query_set_filter(query, filter);
...
PersonProvider::DataList::iterator PersonProvider::findByPersonId(int personId)
{
	return std::find_if(m_PersonList.begin(), m_PersonList.end(),
		[personId](ContactData *contactData) {
			return contactData->getId() == personId;
		}
	);
}

PersonProvider::DataList::iterator PersonProvider::findByContactId(int contactId)
{
	return std::find_if(m_PersonList.begin(), m_PersonList.end(),
		[contactId](ContactData *contactData) {
			Person *person = static_cast<Person *>(contactData);
			return person->getContactId() == contactId;
		}
	);
}

void PersonProvider::insertPerson(int contactId)
{
	contacts_record_h record = getPersonRecord(_contacts_person.display_contact_id, contactId);
	if (record) {
		m_PersonList.push_back(createPerson(record));
...
void PersonProvider::updatePerson(DataList::iterator personIt)
{
	...
	contacts_record_h record = getPersonRecord(_contacts_person.id, person->getId());
...
void PersonProvider::updatePersonList()
{
	...
	for (auto &&record : makeRange(changes)) {
		...
		switch (changeType) {
			...
			{
				insertPerson(contactId);
				break;
			}
			case CONTACTS_CHANGE_UPDATED:
			{
				auto personIt = findByPersonId(getPersonId(contactId));
			...
			{
				auto personIt = findByContactId(contactId);
				if (personIt != m_PersonList.end()) {
					deletePerson(personIt);
				}
				break;
