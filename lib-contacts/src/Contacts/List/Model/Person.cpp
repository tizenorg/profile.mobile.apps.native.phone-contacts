...
{
	contacts_record_h record  = getContactChildRecord(_contacts_contact.nickname, [](contacts_record_h record) -> bool {
			return getRecordStr(record, _contacts_nickname.name);
		}
	);
...
{
	contacts_record_h record  = getContactChildRecord(_contacts_contact.note, [](contacts_record_h record) -> bool {
			return getRecordStr(record, _contacts_note.note);
		}
	);
...
{
	return getContactChildRecord(_contacts_contact.company, [](contacts_record_h record) -> bool {
		return getRecordStr(record, _contacts_company.name) || getRecordStr(record, _contacts_company.job_title);
		}
	);
}
...
const Person::ContactChildRecords Person::getContactChildRecords(unsigned propertyId) const
{
	...
	for (auto &&contact : m_ContactRecords) {
		records.emplace_back(makeRange(contact, propertyId));
	}
