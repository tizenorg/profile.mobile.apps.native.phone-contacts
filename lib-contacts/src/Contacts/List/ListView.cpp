...
void ListView::onNameDisplayOrderChanged(contacts_name_display_order_e displayOrder, void *data)
{
	RETM_IF(!data, "invalid data");
	ListView *view = (ListView *)data;
...
void ListView::updatePersonItem(PersonItem *item, int changes)
{
	if (changes & Person::ChangedSortValue) {
		...
		int partChanges = ELM_GENLIST_ITEM_FIELD_ALL;
		if (changes & ContactData::ChangedImage) {
			partChanges |= ELM_GENLIST_ITEM_FIELD_CONTENT;
		}

		if (changes & ContactData::ChangedName) {
			partChanges |= ELM_GENLIST_ITEM_FIELD_TEXT;
		}

		item->updateItemPart(partChanges);
	}
}
