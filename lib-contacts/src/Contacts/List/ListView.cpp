...
ListView::ListView(Model::PersonProvider *provider)
	: m_Box(nullptr), m_NoContent(nullptr), m_Genlist(nullptr),
	  ...
	  m_IsAddButton(true), m_SearchItem(nullptr), m_PersonProvider(provider),
...
void ListView::enableAddButton(bool isEnabled)
{
	m_IsAddButton = isEnabled;
	updateAddButton();
}
...
void ListView::updateAddButton()
{
	...
	if (m_IsCurrentView && getSelectMode() == SelectNone && m_IsAddButton) {
