...
char *PersonSearchItem::getText(Evas_Object *parent, const char *part)
{
	if (strcmp(part, PART_CONTACT_NAME) == 0) {
		auto &searchData = static_cast<PersonSearchData &>(getContactData());
		return getStr(std::bind(&PersonSearchData::getName, std::ref(searchData)),
		...
		PersonSearchData::MatchedField matchedField) const
{
	...
	if (str) {
		auto &searchData = static_cast<PersonSearchData &>(getContactData());
		if (searchData.getMatchedField() == matchedField) {
			return strdup(highlightStr(str, searchData.getSubstring()).c_str());
		} else {
			return strdup(str);
		}
	}
...
{
	auto &searchData = static_cast<PersonSearchData &>(getContactData());
	ContactData *subField = searchData.getFoundSubField();
