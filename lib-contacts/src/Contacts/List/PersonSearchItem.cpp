...
PersonSearchItem::PersonSearchItem(Person &person, Model::PersonSearchData *searchData)
	: PersonItem(person), m_SearchData(searchData)
{
	m_SearchData->setChangedCallback(std::bind(&PersonSearchItem::onSearchDataChanged, this));
}
...
Eina_Bool PersonSearchItem::compare(Evas_Object *parent, void *filter)
{
	...
	bool isExist = (bool)searchResult;
