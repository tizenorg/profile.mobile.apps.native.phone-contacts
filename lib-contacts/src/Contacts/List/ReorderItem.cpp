...
Evas_Object *ReorderItem::getContent(Evas_Object *parent, const char *part)
{
	if (strcmp(part, PART_CONTACT_THUMBNAIL) == 0) {
			Ui::Thumbnail *thumbnail = Ui::Thumbnail::create(parent, Ui::Thumbnail::SizeSmall,
					getContactData().getImagePath());
			thumbnail->setSizeHint(true);
			return thumbnail->getEvasObject();

	} else if (strcmp(part, PART_CONTACT_REORDER) == 0) {
		Evas_Object *button = elm_button_add(parent);
		elm_object_style_set(button, "icon_reorder");
		evas_object_event_callback_add(button, EVAS_CALLBACK_MOUSE_DOWN, makeCallback(&ReorderItem::onMouseDown), this);
