...
void ManageFavoritesPopup::onRemove()
{
	...
	//todo Implement separate controller to handle callback, becase object is destroyed after popup close.
	auto onMfcUpdated = std::move(m_OnMfcUpdated);
	view->setSelectCallback([onMfcUpdated](Ux::SelectResults results) {

		bool mcfUpdate = false;
		contacts_list_h list = nullptr;
		contacts_list_create(&list);
		for (auto &&result : results) {
			contacts_record_h record = nullptr;
