...
namespace Contacts
{
	namespace List
	{
		...
		private:
			...
			ContactGroupItem *prependGroupItem(const char *indexLetter);
			ContactGroupItem *insertGroupItem(const char *indexLetter,
					ContactGroupItem *prevGroup = nullptr);
			ContactGroupItem *getPrevGroupItem(const char *indexLetter);

			ContactItem *getPrevItem(ContactGroupItem *group, Model::Contact *contact);
