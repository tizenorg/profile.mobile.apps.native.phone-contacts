...
ContactFieldPtr ContactArray::getField(unsigned index) const
{
	...
	if (record) {
		return ContactFactory::createField(record, getId(), getMetadata()->element);
	}
...
{
	contacts_record_h record = nullptr;
	contacts_record_create(getUri(), &record);
	contacts_record_add_child_record(getRecord(), getPropertyId(), record);

	ContactFieldPtr field = ContactFactory::createField(record, getId(), getMetadata()->element);
	field->initialize();
	return field;
}
...
const ContactArrayMetadata *ContactArray::getMetadata() const
{
	return (const ContactArrayMetadata *) ContactField::getMetadata();
}
