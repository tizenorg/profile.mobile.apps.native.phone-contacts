...
int getStorageId(storage_type_e storageType)
{
	...
	RETVM_IF(error != STORAGE_ERROR_NONE, -1, "storage_foreach_device_supported() failed");

	return storageData.m_StorageId;
}

std::string Common::getRootDirectoryPath(storage_type_e storageType)
{
	char *dirPath = nullptr;

	int error = storage_get_root_directory(getStorageId(storageType), &dirPath);
	if(STORAGE_ERROR_NONE == error && dirPath) {
		std::string res = dirPath;
		free(dirPath);
		return res;
	} else {
		ERR("storage_foreach_device_supported() failed");
	}
...
std::string Common::getDirectoryPath(storage_type_e storageType, storage_directory_e dirType)
{
	...
	int error = storage_get_directory(getStorageId(storageType), dirType, &dirPath);
	if(STORAGE_ERROR_NONE == error && dirPath) {
		std::string res = dirPath;
		free(dirPath);
		return res;
