...
namespace
{
	...
	const struct {
		...
		/* [EnumTypeImage]        = */ { nullptr, 0 },
		/* [EnumTypeNumber]       = */ { ARRAY(numberValueNames) },
		/* [EnumTypeEmail]        = */ { ARRAY(emailValueNames) },
		/* [EnumTypeAddress]      = */ { ARRAY(addressValueNames) },
		/* [EnumTypeEvent]        = */ { ARRAY(eventValueNames) },
		/* [EnumTypeRelationship] = */ { ARRAY(relValueNames) },
		/* [EnumTypeUrl]          = */ { ARRAY(urlValueNames) },
		/* [EnumTypeMessenger]    = */ { ARRAY(msgValueNames) }
	};
}
...
{
	if (type < Utils::count(enumValueNames)) {
		auto names = &enumValueNames[type];
		return { names->array, names->count };
	}
