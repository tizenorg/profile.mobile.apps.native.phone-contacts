...
void ListView::setMode(Mode mode)
{
	if (m_ViewMode != mode) {
		...
		for (size_t i = SectionSearch; i < SectionMax; ++i) {
...
{
	m_Sections[SectionMyProfile] = new GroupItem("IDS_PB_HEADER_ME");
	m_Genlist->insert(m_Sections[SectionMyProfile], nullptr, getNextGroupItem(SectionMyProfile));
	elm_genlist_item_select_mode_set(m_Sections[SectionMyProfile]->getObjectItem(), ELM_OBJECT_SELECT_MODE_NONE);
}

Ui::GenlistGroupItem *ListView::getNextGroupItem(SectionId currentSection)
{
