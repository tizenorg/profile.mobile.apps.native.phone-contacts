...
{
	auto updateMyProfileItem = [](const char* view_uri, void* user_data) {
		ListView *self = (ListView *) user_data;

		elm_genlist_item_subitems_clear(self->m_MyProfileGroup->getObjectItem());

		MyProfileItem *item = new MyProfileItem(MyProfilePtr(new MyProfile()));
		self->m_Genlist->insert(item, self->m_MyProfileGroup);

		item->setSelectedCallback([self, item]() {
			int id = 0;
			contacts_record_get_int(item->getMyProfile().getRecord(), _contacts_my_profile.id, &id);

			self->getNavigator()->navigateTo(new Input::InputView(id, Input::InputView::TypeMyProfile));
		});
	};
	...
	updateMyProfileItem(nullptr, this);

	contacts_db_add_changed_cb(_contacts_my_profile._uri, updateMyProfileItem, this);
}
