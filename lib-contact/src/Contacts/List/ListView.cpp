...
Elm_Object_Item *ListView::insertIndex(const char *indexLetter, Elm_Object_Item *nextItem)
{
...
{
	Elm_Object_Item *indexItem = insertIndex(indexLetter, nextGroup ? nextGroup->getIndexItem() : nullptr);
...
void ListView::updateContactItem(Model::ContactPtr contact, ContactItem *item)
{
...
//		Todo: Implement delete contact functionality
		...
		updateContactItem(std::move(contact), item);
	}
}
