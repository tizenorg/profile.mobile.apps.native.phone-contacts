...
ContactData *PersonProvider::createContact(contacts_record_h record)
{
...
void PersonProvider::updateChangedCallback(ContactDataList::iterator personIt)
{
	...
	person->unsetChangedCallback();
	person->setChangedCallback(
			std::bind(&PersonProvider::onChanged, this, personIt, _1, _2));
}
