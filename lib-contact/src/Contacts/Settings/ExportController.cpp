...
std::string ExportController::getVcardRelativeRootPath() const
{
	...
	endPosition += deviceDir.length();

	char vcardPosition[BUFFER_SIZE] = { 0, };
	size_t length = m_VcardPath.copy(vcardPosition, (m_VcardPath.length() - endPosition), (endPosition + 1));
	vcardPosition[length]='\0';

	return vcardPosition;
}
