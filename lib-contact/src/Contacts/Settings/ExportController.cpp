...
{
	RETM_IF((storageParam[m_VcardStorage].directory != STORAGE_DIRECTORY_MAX),
			"Wrong vcard storage");

	std::string path = getDirectoryPath(storageParam[m_VcardStorage].type, storageParam[m_VcardStorage].directory);
