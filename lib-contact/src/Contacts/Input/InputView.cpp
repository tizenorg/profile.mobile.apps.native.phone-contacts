...
{
	ContactObjectItem *mainItem = nullptr;
	if (field->getSubType() & ObjectTyped) {
		mainItem = new ContactTypedObjectItem(std::move(field));
	} else {
		mainItem = new ContactObjectItem(std::move(field));
	}

	using namespace std::placeholders;
	mainItem->setRemoveCallback([this](ContactObjectItem *item, ContactFieldPtr field) {
...
void InputView::removeFieldItem(ContactObjectItem *item, unsigned fieldId)
{
	if (item == m_Items[fieldId]) {
		...
		unsigned nextFieldId = nextItem ? nextItem->getField()->getId() : FieldEnd;
		m_Items[fieldId] = (nextFieldId == fieldId) ? nextItem : nullptr;
	}
}
